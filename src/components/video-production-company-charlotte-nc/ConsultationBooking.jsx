...
export default function ConsultationSection() {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      setNowTime(now.toLocaleString("en-US", opts));
    };
    tick();
  ...
  const prevMonth = () => {
    ...
    todayStart.setDate(1); todayStart.setHours(0,0,0,0);
    if (d >= new Date(todayStart.getFullYear(), todayStart.getMonth(), 1)) {
      setCalendarDate(d);
      setSelectedDay(null);
  ...
  return (
    <section className="font-['Segoe_UI',Arial,sans-serif] bg-[#0a0a0a] min-h-screen text-white">
      {/* Top Banner */}
      <div className="bg-[#111] text-center pt-12 px-6 pb-10 border-b-2 border-[#cc0000]">
        <h1 className="text-[clamp(18px,3vw,28px)] font-black tracking-[0.05em] text-white mb-3 uppercase">
      ...
      {/* Real-time clock bar */}
      <div className="bg-[#1a1a1a] border-b border-[#2a2a2a] py-2.5 px-8 flex items-center gap-2 text-[13px] text-[#ccc]">
        <span className="text-base">🕐</span>
        <span className="font-semibold text-white">{nowTime}</span>
        <span className="text-[#cc0000] font-medium"> — {selectedTimezone.label}</span>
      </div>

      {/* Booking Widget */}
      <div id="booking-widget" className="max-w-[900px] my-10 mx-auto bg-white rounded-lg overflow-visible shadow-[0_8px_40px_rgba(0,0,0,0.5)]">
        {/* Tabs */}
        <div className="flex border-b-2 border-[#eee]">
          <button
            className={`flex-1 p-4 bg-none border-none text-[15px] font-semibold cursor-pointer border-b-[3px] transition-all duration-200 ${
              activeTab === "availability" ? "text-[#cc0000] border-b-[#cc0000]" : "text-[#555] border-b-transparent"
            }`}
          ...
          <button
            className={`flex-1 p-4 bg-none border-none text-[15px] font-semibold cursor-pointer border-b-[3px] transition-all duration-200 ${
              activeTab === "form" ? "text-[#cc0000] border-b-[#cc0000]" : "text-[#555] border-b-transparent"
            }`}
        ...
        {activeTab === "availability" && (
          <div className="flex gap-0 min-h-[480px]">
            {/* Left: Calendar */}
            <div className="flex-1 py-7 px-6 border-r border-[#eee]">
              <h2 className="text-[20px] font-extrabold text-[#111] mb-1">Book a Zoom Meeting</h2>
              <p className="text-[#cc0000] font-bold text-sm mb-1.5">Final Cut Multimedia</p>
              <p className="text-xs text-[#888] mb-4">⏱ Appointment Duration: 15 minutes</p>

              {/* Timezone selector */}
              <div className="relative mb-5">
                <button className="bg-[#f5f5f5] border border-[#ddd] py-[7px] px-3.5 rounded-[4px] text-xs font-semibold cursor-pointer text-[#333] whitespace-nowrap overflow-hidden text-ellipsis max-w-full" onClick={() => setShowTzDropdown(!showTzDropdown)}>
                  🌐 {selectedTimezone.label} ▾
                </button>
                {showTzDropdown && (
                  <div className="absolute top-[calc(100%+4px)] left-0 z-[100] bg-white border border-[#ddd] rounded-md shadow-[0_4px_16px_rgba(0,0,0,0.15)] min-w-[240px] max-h-[220px] overflow-y-auto">
                    {TIMEZONES.map((tz) => (
                      <div
                        key={tz.value}
                        className={`py-2.5 px-3.5 text-[13px] cursor-pointer text-[#333] transition-colors duration-150 hover:bg-gray-100 ${
                          tz.value === selectedTimezone.value ? "bg-[#fff0f0] text-[#cc0000] font-bold hover:bg-[#fff0f0]" : ""
                        }`}
              ...
              <div className="flex items-center justify-between mb-3">
                <button
                  className={`bg-none border border-[#ddd] w-8 h-8 rounded-full text-[18px] flex items-center justify-center text-[#333] leading-none ${
                    isPrevMonthDisabled() ? "opacity-30 cursor-not-allowed" : "cursor-pointer"
                  }`}
                  onClick={prevMonth}
              ...
              <div className="grid grid-cols-7 gap-1">
                ...
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
                  ...
                  return (
                    <div
                      key={day}
                      className={`text-center py-2 px-1 rounded-full text-[13px] cursor-pointer font-medium transition-all duration-150 select-none ${
                        past ? "text-[#ccc] cursor-not-allowed" : isSelected ? "bg-[#cc0000] text-white font-bold" : "text-[#111] font-bold hover:bg-gray-100"
                      }`}
            ...
            {/* Right: Time Slots */}
            {selectedDay && (
              <div className="w-[200px] py-7 px-4 flex flex-col gap-2">
                <div className="mb-2">
                  <p className="font-extrabold text-base text-[#111] m-0">{dayName}</p>
                  <p className="text-xs text-[#888] mt-0.5">{fullDateStr}</p>
                </div>
                <div className="flex-1 overflow-y-auto flex flex-col gap-1.5 max-h-[320px] pr-[2px]">
                  {HOURS.map((h) => (
                    <button
                      key={h}
                      className={`bg-white border-[1.5px] border-[#ddd] rounded-[4px] py-[9px] px-2 text-[13px] font-semibold cursor-pointer text-[#333] transition-all duration-150 text-center ${
                        selectedTime === h ? "bg-[#cc0000] border-[#cc0000] text-white" : "hover:border-[#cc0000]"
                      }`}
                ...
                <button
                  className={`border-none rounded-[4px] py-3 px-2 font-bold text-[13px] mt-2 tracking-wide ${
                    !selectedTime ? "bg-[#e8b4b4] cursor-not-allowed text-white" : "bg-[#cc0000] text-white cursor-pointer"
                  }`}
            ...
            {!selectedDay && (
              <div className="flex-1 flex items-center justify-center text-[#bbb] text-sm p-5">
          ...
          <div className="pt-7 px-8 pb-8 text-[#111]">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1">
                ...
                <input 
                  className={`border-[1.5px] border-[#ddd] rounded-[4px] py-2.5 px-3 text-[13px] text-[#111] bg-[#fafafa] outline-none transition-colors duration-150 font-inherit ${
                    errors.company ? "border-[#cc0000] bg-[#fff8f8]" : "focus:border-gray-400"
                  }`}
              ...
              <div className="flex flex-col gap-1">
                ...
                <input 
                  className={`border-[1.5px] border-[#ddd] rounded-[4px] py-2.5 px-3 text-[13px] text-[#111] bg-[#fafafa] outline-none transition-colors duration-150 font-inherit ${
                    errors.firstName ? "border-[#cc0000] bg-[#fff8f8]" : "focus:border-gray-400"
                  }`}
              ...
              <div className="flex flex-col gap-1">
                ...
                <input 
                  className={`border-[1.5px] border-[#ddd] rounded-[4px] py-2.5 px-3 text-[13px] text-[#111] bg-[#fafafa] outline-none transition-colors duration-150 font-inherit ${
                    errors.lastName ? "border-[#cc0000] bg-[#fff8f8]" : "focus:border-gray-400"
                  }`}
              ...
              <div className="flex flex-col gap-1">
                ...
                <input 
                  className={`border-[1.5px] border-[#ddd] rounded-[4px] py-2.5 px-3 text-[13px] text-[#111] bg-[#fafafa] outline-none transition-colors duration-150 font-inherit ${
                    errors.phone ? "border-[#cc0000] bg-[#fff8f8]" : "focus:border-gray-400"
                  }`}
              ...
              <div className="flex flex-col gap-1">
                ...
                <input 
                  className={`border-[1.5px] border-[#ddd] rounded-[4px] py-2.5 px-3 text-[13px] text-[#111] bg-[#fafafa] outline-none transition-colors duration-150 font-inherit ${
                    errors.email ? "border-[#cc0000] bg-[#fff8f8]" : "focus:border-gray-400"
                  }`}
              ...
              <div className="flex flex-col gap-1 col-span-2">
                ...
                <textarea
                  className={`border-[1.5px] border-[#ddd] rounded-[4px] py-2.5 px-3 text-[13px] text-[#111] bg-[#fafafa] outline-none transition-colors duration-150 font-inherit resize-y min-h-[90px] ${
                    errors.projectDetails ? "border-[#cc0000] bg-[#fff8f8]" : "focus:border-gray-400"
                  }`}
              ...
              <div className="flex flex-col gap-1 col-span-2">
                ...
                <select
                  className={`border-[1.5px] border-[#ddd] rounded-[4px] py-2.5 px-3 text-[13px] text-[#111] bg-[#fafafa] outline-none transition-colors duration-150 font-inherit appearance-auto cursor-pointer ${
                    errors.howHeard ? "border-[#cc0000] bg-[#fff8f8]" : "focus:border-gray-400"
                  }`}
                  value={form.howHeard}
            ...
            <div className="flex justify-between items-center mt-7 pt-5 border-t border-[#eee]">
              ...
              <button className="bg-[#cc0000] text-white border-none rounded-[4px] py-3 px-8 font-extrabold text-sm tracking-widest cursor-pointer uppercase hover:bg-[#b30000]" onClick={handleBookMeeting}>
                BOOK MEETING
              </button>
            </div>
          </div>
